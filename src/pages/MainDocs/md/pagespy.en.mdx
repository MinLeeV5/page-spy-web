...
Environment identifier used for room filtering and display in the debug room list.

- Type:

  ```ts
  type RoomEnv = 'dev' | 'test' | 'uat' | 'prod';
  ```
...
Application version identifier used for room filtering and display in the debug room list.
...
- Example
  ...
  async function YourCode() {
    const { title, project, env, version } = await xxx();
